...
local function jsonToModel(jsonCmd, jsonType)
    ...
    if (streams["linkage_2_swing_control_type"] ~= nil) then keyP["linkage_2_swing_control_type"] = streams
        ["linkage_2_swing_control_type"] end
    if (streams["linkage_2_dehumidity_control_type"] ~= nil) then keyP["linkage_2_dehumidity_control_type"] = streams
        ["linkage_2_dehumidity_control_type"] end
    if (streams["linkage_2_remove_arofene_control_type"] ~= nil) then keyP["linkage_2_remove_arofene_control_type"] =
        streams["linkage_2_remove_arofene_control_type"] end
    if (streams["linkage_2_remove_allergy_control_type"] ~= nil) then keyP["linkage_2_remove_allergy_control_type"] =
        streams["linkage_2_remove_allergy_control_type"] end
    if (streams["linkage_2_air_remove_odor_control_type"] ~= nil) then keyP["linkage_2_air_remove_odor_control_type"] =
        streams["linkage_2_air_remove_odor_control_type"] end
    ...
    if (jsonType == "control" and (streams["prevent_super_cool_2"] ~= nil or streams["prevent_super_cool_mode_select"] ~= nil)) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["prevent_super_cool_control"] = 1
    end
    if (streams["prevent_super_cool_2"] ~= nil) then keyP["prevent_super_cool_2"] = streams["prevent_super_cool_2"] end
    if (streams["prevent_super_cool_mode_select"] ~= nil) then keyP["prevent_super_cool_mode_select"] = streams
        ["prevent_super_cool_mode_select"] end
    if (jsonType == "control" and streams["radar_model_control"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["radar_model_control"] = streams["radar_model_control"]
    end
    if (streams["dynamic_distance_check"] ~= nil) then keyP["dynamic_distance_check"] = streams
        ["dynamic_distance_check"] end
    if (streams["static_distance_check"] ~= nil) then keyP["static_distance_check"] = streams["static_distance_check"] end
    if (streams["detection_sense_level"] ~= nil) then keyP["detection_sense_level"] = streams["detection_sense_level"] end
    if (streams["report_config"] ~= nil) then keyP["report_config"] = streams["report_config"] end
    if (jsonType == "control" and streams["radar_zone_select"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["radar_zone_select"] = streams["radar_zone_select"]
    end
    if (streams["exit_time_set"] ~= nil) then keyP["exit_time_set"] = streams["exit_time_set"] end
    if (jsonType == "control" and streams["remove_allergy"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["remove_allergy"] = streams["remove_allergy"]
    end
    if (streams["remove_allergy_fan_speed"] ~= nil) then keyP["remove_allergy_fan_speed"] = streams
        ["remove_allergy_fan_speed"] end
    ...
    if (streams["remove_arofene_fan_speed"] ~= nil) then keyP["remove_arofene_fan_speed"] = streams
        ["remove_arofene_fan_speed"] end
...
local function binToModel(binData, deviceSN8)
    ...
    if (dataType == 0x05 and messageBytes[0] == 0xB5) then
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if (messageBytes[cursor + 0] == 0xE2 and messageBytes[cursor + 1] == 0x00) then
                ...
                keyP["water_tank_protect"] = bit.rshift(bit.band(messageBytes[cursor + 55], 0x80), 7)
                keyP["analysis_value"] = keyP["analysis_value"] ..
            ...
            if (messageBytes[cursor + 0] == 0xE0 and messageBytes[cursor + 1] == 0x00) then
                keyP["tvoc_density"] = messageBytes[cursor + 23] * 256 + messageBytes[cursor + 22]
                ...
                if (keyP["outdoor_temperature"] > 32767) then keyP["outdoor_temperature"] = keyP["outdoor_temperature"] -
                    65535 - 1 end
                keyP["pm25_value"] = messageBytes[cursor + 21] * 256 + messageBytes[cursor + 20]
                keyP["fresh_air_intake_temp"] = messageBytes[cursor + 19] * 256 + messageBytes[cursor + 18]
                if (keyP["fresh_air_intake_temp"] > 32767) then keyP["fresh_air_intake_temp"] = keyP
                    ["fresh_air_intake_temp"] - 65535 - 1 end
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            ...
            if (messageBytes[cursor + 0] == 0xD3 and messageBytes[cursor + 1] == 0x00) then
                keyP["imax_wind"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xD2 and messageBytes[cursor + 1] == 0x00) then
                keyP["prevent_super_cool_2"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xD4 and messageBytes[cursor + 1] == 0x00) then
                keyP["current_radar_status"] = messageBytes[cursor + 8]
                keyP["millimeter_wave_model"] = messageBytes[cursor + 9]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xD5 and messageBytes[cursor + 1] == 0x00) then cursor = cursor +
                messageBytes[cursor + 2] + 3 end
            if (messageBytes[cursor + 0] == 0xD6 and messageBytes[cursor + 1] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xD7 and messageBytes[cursor + 1] == 0x00) then
                keyP["radar_zone_select"] = messageBytes[cursor + 3]
                keyP["exit_time_set"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xD8 and messageBytes[cursor + 1] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xD9 and messageBytes[cursor + 1] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xCF and messageBytes[cursor + 1] == 0x00) then
                keyP["remove_allergy"] = messageBytes[cursor + 3]
                keyP["remove_allergy_fan_speed"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xD1 and messageBytes[cursor + 1] == 0x00) then
                keyP["remove_arofene"] = messageBytes[cursor + 3]
                keyP["remove_arofene_fan_speed"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xDA and messageBytes[cursor + 1] == 0x00) then
                keyP["cx2_clean_breath"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xDB and messageBytes[cursor + 1] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0x7E and messageBytes[cursor + 1] == 0x00) then
                ...
                keyP["high_temp_remove_odor_alone"] = bit.rshift(bit.band(messageBytes[cursor + 25], 0x02), 1)
                keyP["self_remove_odor_phase"] = bit.rshift(bit.band(messageBytes[cursor + 25], 0x04), 2)
                if (keyP["wind_swing_ud_angle"] == 2) then keyP["wind_swing_ud_angle"] = 13 elseif (keyP["wind_swing_ud_angle"] == 3) then keyP["wind_swing_ud_angle"] = 25 elseif (keyP["wind_swing_ud_angle"] == 4) then keyP["wind_swing_ud_angle"] = 38 elseif (keyP["wind_swing_ud_angle"] == 5) then keyP["wind_swing_ud_angle"] = 50 elseif (keyP["wind_swing_ud_angle"] == 6) then keyP["wind_swing_ud_angle"] = 62 elseif (keyP["wind_swing_ud_angle"] == 7) then keyP["wind_swing_ud_angle"] = 75 elseif (keyP["wind_swing_ud_angle"] == 8) then keyP["wind_swing_ud_angle"] = 88 elseif (keyP["wind_swing_ud_angle"] == 9) then keyP["wind_swing_ud_angle"] = 100 end
                ...
                if (messageBytes[cursor + 3] > 50) then
                    ...
                    keyP["imax_wind"] = bit.rshift(bit.band(messageBytes[cursor + 54], 0x80), 7)
                end
                ...
                if (messageBytes[cursor + 3] > 55) then keyP["humidification_fan_speed"] = messageBytes[cursor + 59] end
                if (messageBytes[cursor + 3] > 57) then keyP["wetfilm_time_use"] = messageBytes[cursor + 61] * 256 +
                    messageBytes[cursor + 60] end
                if (messageBytes[cursor + 3] > 58) then
                    keyP["humidification"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x02), 1)
                    keyP["humidification_mode"] = bit.band(messageBytes[cursor + 62], 0x01)
                    keyP["wet_film_timeout"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x04), 2)
                    keyP["linkage_2_swing_auto"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x08), 3)
                    keyP["linkage_2_dehumidity_auto"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x10), 4)
                    keyP["prevent_super_cool_2"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x40), 6)
                end
                if (messageBytes[cursor + 3] > 59) then
                    keyP["remove_allergy"] = bit.rshift(bit.band(messageBytes[cursor + 63], 0x80), 7)
                    keyP["remove_allergy_fan_speed"] = bit.band(messageBytes[cursor + 63], 0x7F)
                end
                if (messageBytes[cursor + 3] > 60) then
                    keyP["remove_arofene"] = bit.rshift(bit.band(messageBytes[cursor + 64], 0x80), 7)
                    keyP["remove_arofene_fan_speed"] = bit.band(messageBytes[cursor + 64], 0x7F)
                end
                if (messageBytes[cursor + 3] > 63) then
                    keyP["remove_allergy_filter"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x02), 1)
                    keyP["remove_arofene_filter"] = bit.band(messageBytes[cursor + 67], 0x01)
                    keyP["linkage_2_remove_arofene_auto"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x04), 2)
                    keyP["cx2_clean_breath"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x08), 3)
                    keyP["linkage_2_remove_allergy_auto"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x10), 4)
                    keyP["linkage_2_air_remove_odor_auto"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x20), 5)
                end
                if (messageBytes[cursor + 3] > 65) then keyP["remove_allergy_filter_use_time"] = messageBytes
                    [cursor + 69] * 256 + messageBytes[cursor + 68] end
                if (messageBytes[cursor + 3] > 62) then keyP["air_filter_use_time"] = messageBytes[cursor + 66] * 256 +
                    messageBytes[cursor + 65] end
                if (messageBytes[cursor + 3] > 69) then
                    keyP["main_horizontal_guide_strip_1"] = messageBytes[cursor + 70]
                    keyP["main_horizontal_guide_strip_2"] = messageBytes[cursor + 71]
                    keyP["main_horizontal_guide_strip_3"] = messageBytes[cursor + 72]
                    keyP["main_horizontal_guide_strip_4"] = messageBytes[cursor + 73]
                end
    ...
    if ((dataType == 0x02 and messageBytes[0] == 0xB0) or (dataType == 0x03 and messageBytes[0] == 0xB1)) then
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if (messageBytes[cursor + 0] == 0x80 and messageBytes[cursor + 1] == 0x00) then if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] == 0x10 or messageBytes[cursor + 3] == 0x00) then cursor =
                    cursor + 4 else
            ...
            if (messageBytes[cursor + 0] == 0x82 and messageBytes[cursor + 1] == 0x00) then if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] == 0x10 or messageBytes[cursor + 3] == 0x00) then cursor =
                    cursor + 4 else
            ...
            if (messageBytes[cursor + 0] == 0xE2 and messageBytes[cursor + 1] == 0x00) then
                ...
                keyP["water_tank_protect"] = bit.rshift(bit.band(messageBytes[cursor + 56], 0x80), 7)
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xE1 and messageBytes[cursor + 1] == 0x00) then cursor = cursor +
                messageBytes[cursor + 3] + 4 end
            if (messageBytes[cursor + 0] == 0xE0 and messageBytes[cursor + 1] == 0x00) then
                keyP["tvoc_density"] = messageBytes[cursor + 24] * 256 + messageBytes[cursor + 23]
                keyP["outdoor_temperature"] = messageBytes[cursor + 14] * 256 + messageBytes[cursor + 13]
                if (keyP["outdoor_temperature"] > 32767) then keyP["outdoor_temperature"] = keyP["outdoor_temperature"] -
                    65535 - 1 end
                keyP["pm25_value"] = messageBytes[cursor + 22] * 256 + messageBytes[cursor + 21]
                keyP["fresh_air_intake_temp"] = messageBytes[cursor + 20] * 256 + messageBytes[cursor + 19]
                if (keyP["fresh_air_intake_temp"] > 32767) then keyP["fresh_air_intake_temp"] = keyP
                    ["fresh_air_intake_temp"] - 65535 - 1 end
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            ...
            if (messageBytes[cursor + 0] == 0xAE and messageBytes[cursor + 1] == 0x00) then if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] == 0x10 or messageBytes[cursor + 3] == 0x00) then cursor =
                    cursor + 4 else
            ...
            if (messageBytes[cursor + 0] == 0xD2 and messageBytes[cursor + 1] == 0x00) then if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] == 0x10 or messageBytes[cursor + 3] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xD3 and messageBytes[cursor + 1] == 0x00) then if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] == 0x10 or messageBytes[cursor + 3] == 0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xDE and messageBytes[cursor + 1] == 0x00) then
                keyP["fresh_air_filter_total_time"] = messageBytes[cursor + 5] * 256 + messageBytes[cursor + 4]
                keyP["remove_arofene_filter_total_time"] = messageBytes[cursor + 7] * 256 + messageBytes[cursor + 6]
                keyP["remove_allergy_filter_total_time"] = messageBytes[cursor + 9] * 256 + messageBytes[cursor + 8]
                if (messageBytes[cursor + 3] > 6) then keyP["wet_film_filter_total_time"] = messageBytes[cursor + 11] *
                    256 + messageBytes[cursor + 10] end
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0x7E and messageBytes[cursor + 1] == 0x00) then
                if ((function(t)
                ...
                if (messageBytes[cursor + 3] > 63) then
                    keyP["remove_allergy_filter"] = bit.rshift(bit.band(messageBytes[cursor + 62], 0x02), 1)
                    keyP["remove_arofene_filter"] = bit.band(messageBytes[cursor + 67], 0x01)
                    keyP["linkage_2_remove_arofene_auto"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x04), 2)
                    keyP["cx2_clean_breath"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x08), 3)
                    keyP["linkage_2_remove_allergy_auto"] = bit.rshift(bit.band(messageBytes[cursor + 67], 0x10), 4)
    ...
    elseif (control) then
        ...
        if (keyP["propertyNumber"] == 0) then else
            ...
            if (keyP["linkage_2_temp_control_type"] ~= nil) then
                ...
                bodyBytes[cursor + 9] = keyP["linkage_2_dehumidity_control_type"]
                bodyBytes[cursor + 10] = keyP["linkage_2_remove_arofene_control_type"]
                bodyBytes[cursor + 11] = keyP["linkage_2_remove_allergy_control_type"]
                bodyBytes[cursor + 12] = keyP["linkage_2_air_remove_odor_control_type"]
                cursor = cursor + 13
            end
            ...
            if (keyP["radar_zone_select"] ~= nil) then
                ...
                if (keyP["exit_time_set"] == nil) then bodyBytes[cursor + 4] = 60 else bodyBytes[cursor + 4] = keyP
                    ["exit_time_set"] end
                cursor = cursor + 5
            end
...
function dataToJson(jsonCmd)
    ...
    if (keyP["wetfilm_time_use"] ~= nil) then streams["wetfilm_time_use"] = keyP["wetfilm_time_use"] end
    if (keyP["wet_film_timeout"] ~= nil) then streams["wet_film_timeout"] = keyP["wet_film_timeout"] end
    if (keyP["water_tank_protect"] ~= nil) then streams["water_tank_protect"] = keyP["water_tank_protect"] end
    if (keyP["linkage_2_swing_auto"] ~= nil) then streams["linkage_2_swing_auto"] = keyP["linkage_2_swing_auto"] end
    if (keyP["linkage_2_dehumidity_auto"] ~= nil) then streams["linkage_2_dehumidity_auto"] = keyP
        ["linkage_2_dehumidity_auto"] end
    if (keyP["linkage_2_remove_arofene_auto"] ~= nil) then streams["linkage_2_remove_arofene_auto"] = keyP
        ["linkage_2_remove_arofene_auto"] end
    if (keyP["linkage_2_air_remove_odor_auto"] ~= nil) then streams["linkage_2_air_remove_odor_auto"] = keyP
        ["linkage_2_air_remove_odor_auto"] end
    if (keyP["linkage_2_remove_allergy_auto"] ~= nil) then streams["linkage_2_remove_allergy_auto"] = keyP
        ["linkage_2_remove_allergy_auto"] end
    if (keyP["pm25_value"] ~= nil) then streams["pm25_value"] = keyP["pm25_value"] end
    if (keyP["fresh_air_intake_temp"] ~= nil) then streams["fresh_air_intake_temp"] = keyP["fresh_air_intake_temp"] end
    if (keyP["imax_wind"] ~= nil) then streams["imax_wind"] = keyP["imax_wind"] end
    if (keyP["has_imax_wind"] ~= nil) then streams["has_imax_wind"] = keyP["has_imax_wind"] end
    if (keyP["prevent_super_cool_mode_select"] ~= nil) then streams["prevent_super_cool_mode_select"] = keyP
        ["prevent_super_cool_mode_select"] end
    if (keyP["has_prevent_super_cool_mode_select"] ~= nil) then streams["has_prevent_super_cool_mode_select"] = keyP
        ["has_prevent_super_cool_mode_select"] end
    if (keyP["current_radar_status"] ~= nil) then streams["current_radar_status"] = keyP["current_radar_status"] end
    if (keyP["millimeter_wave_model"] ~= nil) then streams["millimeter_wave_model"] = keyP["millimeter_wave_model"] end
    if (keyP["radar_model_control"] ~= nil) then streams["radar_model_control"] = keyP["radar_model_control"] end
    if (keyP["dynamic_distance_check"] ~= nil) then streams["dynamic_distance_check"] = keyP["dynamic_distance_check"] end
    if (keyP["static_distance_check"] ~= nil) then streams["static_distance_check"] = keyP["static_distance_check"] end
    if (keyP["detection_sense_level"] ~= nil) then streams["detection_sense_level"] = keyP["detection_sense_level"] end
    if (keyP["report_config"] ~= nil) then streams["report_config"] = keyP["report_config"] end
    if (keyP["radar_zone_select"] ~= nil) then streams["radar_zone_select"] = keyP["radar_zone_select"] end
    if (keyP["millimeter_wave_model_fault"] ~= nil) then streams["millimeter_wave_model_fault"] = keyP
        ["millimeter_wave_model_fault"] end
    if (keyP["communication_fault"] ~= nil) then streams["communication_fault"] = keyP["communication_fault"] end
    if (keyP["sense_target"] ~= nil) then streams["sense_target"] = keyP["sense_target"] end
    if (keyP["sense_distance"] ~= nil) then streams["sense_distance"] = keyP["sense_distance"] end
    if (keyP["target1_distance"] ~= nil) then streams["target1_distance"] = keyP["target1_distance"] end
    if (keyP["target1_angle"] ~= nil) then streams["target1_angle"] = keyP["target1_angle"] end
    if (keyP["target2_distance"] ~= nil) then streams["target2_distance"] = keyP["target2_distance"] end
    if (keyP["target2_angle"] ~= nil) then streams["target2_angle"] = keyP["target2_angle"] end
    if (keyP["target3_distance"] ~= nil) then streams["target3_distance"] = keyP["target3_distance"] end
    if (keyP["target3_angle"] ~= nil) then streams["target3_angle"] = keyP["target3_angle"] end
    if (keyP["target4_distance"] ~= nil) then streams["target4_distance"] = keyP["target4_distance"] end
    if (keyP["target4_angle"] ~= nil) then streams["target4_angle"] = keyP["target4_angle"] end
    if (keyP["target5_distance"] ~= nil) then streams["target5_distance"] = keyP["target5_distance"] end
    if (keyP["target5_angle"] ~= nil) then streams["target5_angle"] = keyP["target5_angle"] end
    if (keyP["prevent_super_cool_2"] ~= nil) then streams["prevent_super_cool_2"] = keyP["prevent_super_cool_2"] end
    if (keyP["target1_area_tag"] ~= nil) then streams["target1_area_tag"] = keyP["target1_area_tag"] end
    if (keyP["target2_area_tag"] ~= nil) then streams["target2_area_tag"] = keyP["target2_area_tag"] end
    if (keyP["target3_area_tag"] ~= nil) then streams["target3_area_tag"] = keyP["target3_area_tag"] end
    if (keyP["target4_area_tag"] ~= nil) then streams["target4_area_tag"] = keyP["target4_area_tag"] end
    if (keyP["target5_area_tag"] ~= nil) then streams["target5_area_tag"] = keyP["target5_area_tag"] end
    if (keyP["guide_strip_lr_close"] ~= nil) then streams["guide_strip_lr_close"] = keyP["guide_strip_lr_close"] end
    if (keyP["radar_model_control_status"] ~= nil) then streams["radar_model_control_status"] = keyP
        ["radar_model_control_status"] end
    if (keyP["dynamic_distance_check_status"] ~= nil) then streams["dynamic_distance_check_status"] = keyP
        ["dynamic_distance_check_status"] end
    if (keyP["static_distance_check_status"] ~= nil) then streams["static_distance_check_status"] = keyP
        ["static_distance_check_status"] end
    if (keyP["detection_sense_level_status"] ~= nil) then streams["detection_sense_level_status"] = keyP
        ["detection_sense_level_status"] end
    if (keyP["report_config_status"] ~= nil) then streams["report_config_status"] = keyP["report_config_status"] end
    if (keyP["air_filter_use_time"] ~= nil) then streams["air_filter_use_time"] = keyP["air_filter_use_time"] end
    if (keyP["remove_allergy"] ~= nil) then streams["remove_allergy"] = keyP["remove_allergy"] end
    if (keyP["remove_allergy_fan_speed"] ~= nil) then streams["remove_allergy_fan_speed"] = keyP
        ["remove_allergy_fan_speed"] end
    if (keyP["remove_arofene"] ~= nil) then streams["remove_arofene"] = keyP["remove_arofene"] end
    if (keyP["remove_arofene_fan_speed"] ~= nil) then streams["remove_arofene_fan_speed"] = keyP
        ["remove_arofene_fan_speed"] end
    if (keyP["remove_allergy_filter"] ~= nil) then streams["remove_allergy_filter"] = keyP["remove_allergy_filter"] end
    if (keyP["remove_arofene_filter"] ~= nil) then streams["remove_arofene_filter"] = keyP["remove_arofene_filter"] end
    if (keyP["remove_allergy_filter_use_time"] ~= nil) then streams["remove_allergy_filter_use_time"] = keyP
        ["remove_allergy_filter_use_time"] end
    if (keyP["cx2_clean_breath"] ~= nil) then streams["cx2_clean_breath"] = keyP["cx2_clean_breath"] end
    if (keyP["fresh_air_filter_total_time"] ~= nil) then streams["fresh_air_filter_total_time"] = keyP
        ["fresh_air_filter_total_time"] end
    if (keyP["remove_allergy_filter_total_time"] ~= nil) then streams["remove_allergy_filter_total_time"] = keyP
        ["remove_allergy_filter_total_time"] end
    if (keyP["remove_arofene_filter_total_time"] ~= nil) then streams["remove_arofene_filter_total_time"] = keyP
        ["remove_arofene_filter_total_time"] end
    if (keyP["wet_film_filter_total_time"] ~= nil) then streams["wet_film_filter_total_time"] = keyP
        ["wet_film_filter_total_time"] end
    if (keyP["exit_time_set"] ~= nil) then streams["exit_time_set"] = keyP["exit_time_set"] end
    keyP["propertyNumber"] = 0
    keyP["prevent_super_cool"] = nil
    keyP["prevent_straight_wind"] = nil
    keyP["auto_prevent_straight_wind"] = nil
